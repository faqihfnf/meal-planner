  ...
  return (
    <Collapsible.Root open={open} onOpenChange={setOpen}>
      ...
      <Collapsible.Content forceMount>
        <motion.div
          className={`flex flex-col gap-2 ${open ? "pointer-events-none" : "pointer-events-auto"}`}
        ...
        >
          {items.map((item) => (
            ...
            >
              <Link
                href={item.href}
                className={`flex items-center rounded-md px-5 py-2 transition-all ${pathname === item.href ? "bg-foreground/10 hover:bg-foreground/5" : "hover:bg-foreground/10"}`}
              >
...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  return (
    <div className="flex">
      ...
      <main
        className={`transition-margin mt-12 flex-1 p-4 duration-300 ${open ? "ml-64" : "ml-0"}`}
      >
        {children}
      </main>
    </div>
